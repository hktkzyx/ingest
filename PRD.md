...
按 Phase 划分。**每个 Phase 在此声明目标与产出**；具体待办清单（含已完成 / 进行中 / 待做）见 [TODO.md](./TODO.md) — 它是唯一的 task source of truth，避免两个文件漂移。

### Phase 1 — MVP (v0.1.x)

**目标**：解决核心痛点，可用且安全。

**产出**：CLI 工具能识别设备、推断时间段、按模板归档、流式校验拷贝、SQLite 增量识别、覆盖保护、跨 3 平台 5 架构发版。零参数交互式向导覆盖典型流程。

### Phase 2 — 完善 (v0.2.x)

**目标**：从「能用」提升到「顺手」 — 可配置、可恢复、可审计。

**产出**：交互输入容错、运行日志、`verify` / `history` 子命令、多目标备份、详细报告、`devices.yaml` schema 校验、TUI（bubbletea）。

### Phase 3 — 进阶 (v0.3.x)

**目标**：面向团队 / 专业影像工作流。

**产出**：代理文件转码（FFmpeg）、剪辑软件 XML 导出、多卡队列、WebDAV / NAS / 云端归档目标、并发拷贝性能优化。

### Phase 4 — 稳定 (v1.0.0+)

**目标**：长期可维护的开源项目。

**产出**：测试覆盖率 >80%（含集成测试）、文档网站、官方包管理分发（Homebrew / Scoop / Chocolatey）、社区设备规则贡献流程。
